    ...
    void buttonActionHandler(ActionEvent event) {
    	...
    	if(event.getSource().equals(btn_valider)) {
    		...
    		for (Commande_itemController controller : commande_itemControllers) {
				if(controller.isChanged()) {
					...
					if (i <= 0) {
						...
						alert.setContentText("Les modifications n'ont pas �t� enregist�es. Merci de r��ssayer");
						alert.showAndWait();
					} else {
						...
						alert.setContentText("Modifications enregistr�es avec succ�s");
						alert.showAndWait();
					}
				}
			}
    		if (!found) {
				...
				alert.setContentText("Aucun changement � signaler. Merci de r��ssayer");
				alert.showAndWait();
			}
